...
cd /tmp/update
cp -rf $1 /tmp/update/home_h31m

# hack
# test if bz2 file and jump directly to decompression function

dd if=home_h31m of=header bs=2 count=1
...
if [ "$HEADER" == "5a42" ]; then
    ...
    dd if=home_h31m of=verout bs=22 count=1
    dd if=home_h31m of=home1 bs=22 skip=1 count=999999999999999
    rm home_h31m
    dd if=home1 of=enc_key bs=2048 count=1
    dd if=home1 of=home2 bs=2048 skip=1 count=99999999999999
    rm home1
    ...
    cat verin
    rm home3
    dd if=home4 of=home_h31m.7z bs=22 skip=1 count=9999999999999
    rm home4
    mv home_h31m.7z home4
    cat md5
    zpwd=$(cat pwd)
    ver_out=$(cat verout)
    ver_in=$(cat verin)
...
if [ $act -eq $base ]
then
    echo 7za return pass
else
    echo 7za return fail !!!
    sync
    exit
fi
